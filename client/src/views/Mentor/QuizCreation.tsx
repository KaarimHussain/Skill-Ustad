...
import type { Question, QuizAiRequest, QuizData } from "@/services/quiz.service"
...
export default function CreateQuiz() {
    ...
    const handleSubmit = async () => {
        ...
        const quizId = await QuizService.addQuiz(quizData);

        if (quizId) {
            NotificationService.success("Quiz Created", "Your quiz has been successfully created!");
            navigate("/mentor/quiz")
            // Optionally redirect or reset form here
        } else {
            NotificationService.error("Creation Failed", "There was a problem creating the quiz. Please try again.");
        }
        setIsLoading(false)
    }
            ...
            <div className="relative overflow-hidden">
                ...
                <div className="relative px-4 sm:px-6 lg:px-8 pb-12 pt-25">
                    <div className="max-w-4xl mx-auto">
                        ...
                        <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-4">Create New Quiz</h1>
                        <p className="text-lg text-gray-600 max-w-2xl">
                            Design engaging quizzes that challenge minds and unlock potential for your students
                        </p>
                    </div>
                </div>
            </div>
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex gap-3">
                                <Button onClick={handleSubmit} disabled={isLoading} className="cursor-pointer bg-indigo-500 hover:bg-indigo-600">
                                    {isLoading ? "Saving..." : `Save Quiz as ${quizData.status.status}`}
                                </Button>
                            </div>
                        </div>
                    </CardContent>
