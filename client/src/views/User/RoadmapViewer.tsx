    ...
    const handleVisibilityChange = useCallback(
        async (checked: boolean) => {
            ...
            try {
                ...
                setRoadmapData((prev) => (prev ? { ...prev, visibility: newVisibility } : null))

                // Update Firestore
                const docRef = doc(db, "roadmaps", id)
                await updateDoc(docRef, { visibility: newVisibility })
                console.log(`Roadmap visibility updated to ${newVisibility}`)
            } catch (err) {
                setError("Failed to update roadmap visibility.")
                console.error("Error updating visibility:", err)
                // Revert UI on error
                setRoadmapData((prev) =>
                    prev ? { ...prev, visibility: newVisibility === "public" ? "private" : "public" } : null,
                )
            }
        },
        [roadmapData, id],
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <div className="flex items-center gap-2 cursor-pointer">
                                                    ...
                                                    />
                                                    <label htmlFor="roadmap-visibility" className="cursor-pointer">
                                                        The Roadmap is{" "}
                                                        <span className="font-semibold capitalize">{roadmapData.visibility || "Public"}</span>
                                                    </label>
                                                </div>
                                            </TooltipTrigger>
                                            <TooltipContent>
